...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('global.salesPerson.title_singular') }}
    </div>

    <div class="card-body">
        <form action="{{ route("admin.salesPersons.store") }}" method="POST" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group {{ $errors->has('area') ? 'has-error' : '' }}">
                ...
                <select name="area[]" id="area" class="form-control select2" multiple="multiple">
                    @foreach($areas as $id => $area)
                        <option value="{{ $id }}" {{ (in_array($id, old('area', [])) || isset($salesPerson) && $salesPerson->area->contains($id)) ? 'selected' : '' }}>
                            {{ $area }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="form-group {{ $errors->has('salary') ? 'has-error' : '' }}">
                ...
                <input type="number" value="0.00" id="salary" name="salary" class="form-control" step="0.01">
            ...
            <div class="form-group {{ $errors->has('commissionTargetNotReached') ? 'has-error' : '' }}">
                ...
                <input type="number" value="0.00" id="commissionTargetNotReached" name="commissionTargetNotReached" class="form-control" step="0.01">
            ...
            <div class="form-group {{ $errors->has('commissionTargetReached') ? 'has-error' : '' }}">
                ...
                <input type="number" value="0.00" id="commissionTargetReached" name="commissionTargetReached" class="form-control" step="0.01">
            ...
            <div class="form-group {{ $errors->has('commissionTargetExceeded') ? 'has-error' : '' }}">
                ...
                <input type="number" value="0.00" id="commissionTargetExceeded" name="commissionTargetExceeded" class="form-control" step="0.01">
